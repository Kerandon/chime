...
class AudioManager {
  ...
  AudioPlayer? playerBell;
  AudioPlayer? playerAmbience;

  Future<void> initAudioPlayers() async {
    playerBell = AudioPlayer()..setReleaseMode(ReleaseMode.stop);
    playerAmbience = AudioPlayer()..setReleaseMode(ReleaseMode.loop);
    playerBell!.play(AssetSource('audio/test.mp3'));
    playerAmbience!.play(AssetSource('audio/test.mp3'));
  }
